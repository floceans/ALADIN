...
path_data="/scratch/work/chauvin/UEclimat_2025/{0}/".format(domain)
...
_FillValue=1.e+20
cmp=np.empty([4,10000,2*LARG+1,2*LARG+1])
cmp[:,:,:]=_FillValue

#plevels=[ 1000, 925, 850,700, 600, 500, 400, 300, 250, 200, 150, 100 ]
plevels=[ 850, 700, 500, 300 ]
nlev=len(plevels)
...
# Reading netcdf data 
  var=[]
  for LEV in plevels:
    data_name=data_nam_root+str(LEV)
    file_data=path_data+data_name+'_DOM12_CNRM-CM6-1-HR_amip_r1i1p1f2_CNRM_CNRM-ALADIN63_v1_6hrPt_{1}01010600-{2}01010000.nc'.format(domain,year,yp1)
    g=nc.Dataset(file_data)
    var.append(g.variables[data_name])
#    if LEV == 1000:
    if LEV == 850:
...
# Calculate composites
  ...
  for i in range(0,nrec):
    ...
    print(i,irec,data['time'][i]," ",loncyc,latcyc,indx,indy," ",var[0,data['time'][i]-1,indy,indx])
    if (indy >= (nlat-LARG)):
...
# Writing composites
file_out=path_cmp+'compo_3D_'+data_nam_root+'_'+str(yearmin)+'-'+str(yearmax)+'.'+domain+'.nc'
try:
    os.remove(file_out)
except OSError:
    pass
...
plev= ds.createDimension('plev', nlev)
lat = ds.createDimension('lat', 2*LARG+1)
lon = ds.createDimension('lon', 2*LARG+1)
times = ds.createVariable('time', 'f4', ('time',))
times.units = ''
times.long_name = 'time'
levs = ds.createVariable('plev', 'f4', ('plev',))
levs.units = 'hPa'
levs.long_name = 'pressure levels'
...
tccmp = ds.createVariable('tccmp', 'f4', ('plev', 'time', 'lat', 'lon',),fill_value=_FillValue)
tccmp.units = 'Unknown'
